...
const wchar_t EVENT_NAME[] = L"SvenBXT-Injector";
...
auto GetDLLFileName(std::wstring dll_name)
{
	...
	if (GetModuleFileNameW(NULL, exe_path, MAX_PATH) != 0) {
		...
		fileName += L'\\' + (dll_name.empty() ? L"SvenBXT.dll"s : dll_name);
		return fileName;
	}
	else
...
auto DoInjection(HANDLE targetProcess, std::wstring dll_name)
{
	...
	if (exit_code == 0) {
		std::wcerr << L"LoadLibrary failed. This usually means that you don't have SvenBXT.dll in the same folder as the injector.\n";
		return false;
	}
